...
2. **State Check:** Bunflare queries the Cloudflare API: *"Does 'my-database' exist in this account?"*
...
export default serve({
  async fetch(req) {
    const response = new Response(indexHtml, { headers: { "Content-Type": "text/html" } });
...
app.get("/users/:username", async (c) => {
  ...
  const baseResponse = new Response(indexHtml, { headers: { "Content-Type": "text/html" } });
...
const res = new Response(`<html><head></head><body><div id="app"></div></body></html>`);
...
app.post("/checkout", async (c) => {
  ...
  const result = await env.PAYMENTS.processCharge("user_123", 50.00);
...
export const AnalyticsIngestionQueue = queue({
  ...
  async process(messages, env) {
    ...
    const stmts = messages.map(m =>
      env.DB.prepare("INSERT INTO events (type, payload) VALUES (?, ?)").bind(m.body.type, m.body.payload)
    );
...
export const nightlyCleanup = cron({
  ...
  async run(event, env) {
    ...
    await env.DB.prepare("DELETE FROM sessions WHERE last_active < ?").bind(thirtyDaysAgo).run();
...
export const weeklyReport = cron({
  ...
  async run(event, env) {
    ...
    await sendEmail("admin@company.com", "Weekly Report", JSON.stringify(stats));
  }
});
```
...
export const syncExternalAPI = cron({
  ...
  async run(event, env) {
    const data = await fetch("https://api.external.com/status").then(res => res.json());
...
app.post("/forgot-password", async (c) => {
  ...
  await sendEmail("SEB", { // SEB = Send Email Binding Name
...
app.get("/employees", async (c) => {
  ...
  const { rows } = await db.query("SELECT * FROM employees WHERE active = $1", [true]);
...
app.get("/leaderboard", async (c) => {
  ...
  const topPlayers = await cache.getOrSet("global_leaderboard", { ttl: 60 }, async () => {
    return await db.query("SELECT * FROM players ORDER BY score DESC LIMIT 10");
  });
...
export function getCloudflareContext() {
  if (!_context) throw new Error("Context accessed outside of Request lifecycle");
  return _context;
}
```
...
test("Database inserts correctly", async () => {
  const req = new Request("http://localhost/add-user", { method: "POST", body: JSON.stringify({name: "Alice"})});
...
Not quite. Bunflare uses the same underlying APIs as Wrangler for deployment, but it fundamentally changes the build process. While Wrangler deploys what you configure in TOML/JSON, Bunflare *generates* the configuration based on your TypeScript code. It acts as a meta-framework that compiles down to standard Cloudflare Workers.
...
\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->\n\n<!-- \n  Detailed Technical Documentation Supplement\n  Section: Distributed Tracing & Telemetry Architecture\n  Bunflare natively supports OpenTelemetry to ensure edge requests can be traced\n  across workers and microservices. The context provider propagates the\n  trace context automatically via standard W3C HTTP Headers.\n-->