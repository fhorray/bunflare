...
export async function runBuild(options: { rootDir?: string; production?: boolean } = {}) {
  ...
  console.log(`[bun-cloudflare] 📦 Building ${entrypoints.html.length + 1} entries to ${outdir} (${isProd ? "production" : "development"})...`);
  ...
  if (!workerResult.success) {
    console.error("[bun-cloudflare] ❌ Worker build failed:");
  ...
  if (entrypoints.html.length > 0) {
    ...
    if (!frontendResult.success) {
      console.error("[bun-cloudflare] ❌ Frontend build failed:");
