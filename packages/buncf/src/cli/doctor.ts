...
export async function runDoctor(options: { rootDir?: string } = {}) {
  ...
  console.log(`[buncf] 🩺 Running project diagnostics in ${rootDir}...\n`);
  ...
  const configPath = existsSync(path.join(rootDir, "buncf.config.ts")) 
    ? "buncf.config.ts" 
  ...
  if (configPath) {
    ...
    console.warn(`  ⚠️ Warning: No buncf.config.ts or cloudflare.config.ts found. Using defaults.`);
    warnings++;
  }
  ...
  if (existsSync(wranglerPath) || existsSync(wranglerTomlPath)) {
    ...
    // Parse wrangler.jsonc (simplified)
    if (isJsonc) {
      try {
        const content = readFileSync(wranglerPath, "utf-8");
        // Remove comments for basic parsing
        const cleanJson = content.replace(/\/\/.*/g, "").replace(/\/\*[\s\S]*?\*\//g, "");
  ...
  if (!existsSync(path.join(rootDir, outdir))) {
      console.warn(`  ⚠️ Warning: Output directory "${outdir}" not found. Run "buncf build" first.`);
      warnings++;
  }

  console.log(`\n[buncf] 🏁 Diagnostic complete: ${errors} errors, ${warnings} warnings.`);
  if (errors === 0 && warnings === 0) {
      console.log(`[buncf] ✨ Your project is in perfect health!`);
  } else if (errors === 0) {
      console.log(`[buncf] 💡 Project is functional but could use some optimization.`);
  } else {
      console.error(`[buncf] 🚨 Project has configuration errors that may break functionality.`);
      process.exit(1);
  }
}
