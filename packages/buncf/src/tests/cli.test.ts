...
describe("CLI Integration", () => {
    ...
    writeFileSync(join(testDir, "buncf.config.ts"), `
        export default {
            workerName: "cli-test",
    ...
    it("should run 'build' successfully", () => {
        ...
        expect(stdout).toContain("[buncf] 📦 Building 1 entries to ./dist");
        
        if (result.exitCode === 0) {
            expect(stdout).toContain("[buncf] ✨ Build successful!");
    ...
    it("should show help with 'buncf' name", () => {
        const result = spawnSync(["bun", cliPath, "--help"], {
            cwd: testDir
        });
        const output = result.stdout.toString();
        expect(output).toContain("buncf - CLI para Build");
        expect(output).toContain("bunx buncf <comando>");
    });
