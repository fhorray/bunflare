...
describe("Durable Objects Transformation", () => {
  it("should transform simple durable() to a class", () => {
    ...
    expect(transformed).not.toContain('import { durable } from "buncf"');
  ...
  it("should support function handlers in durable()", () => {
    ...
    expect(transformed).toContain('if (typeof $$handler === "function") return $$handler(request, this.state, this.env)');
  });
  ...
  it("should preserve other imports from buncf in the same line", () => {
    const source = `
      import { getCloudflareContext, durable } from "buncf";
