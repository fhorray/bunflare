          ...
          if (routesProp && routesProp.value.type === "ObjectExpression") {
            ...
            routesProp.value.properties.forEach((prop: any) => {
              ...
              if (prop.value.type === "Identifier") {
                ...
                if (imp && imp.imported !== "*") {
                  routesS.overwrite(pStart, pEnd, `async (req, srv, ctx) => { 
                    const mod = await import("${imp.source}"); 
                    const handler = mod.${imp.imported}; 
                    if (typeof handler === "function") return handler(req, srv, ctx);
                ...
                if (imp && imp.imported === "*") {
                  routesS.overwrite(pStart, pEnd, `async (req, srv, ctx) => { 
                    const mod = await import("${imp.source}"); 
                    const handler = mod.${propertyName}; 
                    if (typeof handler === "function") return handler(req, srv, ctx);
                    return new Response(handler, { headers: { "content-type": "text/html;charset=UTF-8" } });
                  }`);
                }
